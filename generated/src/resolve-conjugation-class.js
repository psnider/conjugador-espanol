...
function findRulesAndProductiveVerbPrefixes(infinitivo, modelo, clase_conjugacional) {
...
function findSharedEnding(infinitivo, modelo) {
    ...
    let terminación = "";
    if (modelo.length > 2) {
        let remainder_modelo = modelo;
        let remainder_infinitivo = infinitivo;
        let does_match = true;
        do {
...
function getInfinitivoSinPrefijos(infinitivo, modelo, prefixes) {
    if (prefixes) {
...
export function resolveConjugationClass(infinitivo) {
    ...
    //     morphological_rules.combinados = combinados
    // }
    let conj_and_deriv_rules = { infinitivo, modelo, infinitivo_sin_prefijos, verb_family, prefixes, morphological_rules };
    conj_and_deriv_rules.cached_tema_pretérito_p3_de_modelo = getIndPretP3StemOfModel(conj_and_deriv_rules);
    return conj_and_deriv_rules;
}
