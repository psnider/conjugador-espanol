...
export function getUnprefixedStems(conj_and_deriv_rules, mood_tense, ancestor_rule_sets, suffixes, rules_applied) {
...
function applicaPrefijosClaseConjugacional(conj_and_deriv_rules, unprefixed_stems, rules_applied) {
...
export function appendSuffixesToStems(infinitivo, stems, regular_suffixes, rules_applied) {
    ...
    function combineStemWithSuffixes(stem_forma, suffix_forms) {
    ...
    function combineStemsWithSuffix(stem_forms, suffix_form) {
    ...
    // 
    function combine2StemsWith2Suffixes(stem_forms, suffix_forms) {
        ...
        if (tipo_combinacional === "secuencial") {
            ...
            for (let stem_i = 0; stem_i < stem_forms.length; ++stem_i) {
                ...
                const formas_combinadas = applyToFormasConjugadas(suffix_forms, (suffix) => {
                    let stem_w_suffix = appendPreferingStressFromSuffix(stem, suffix);
                    return stem_w_suffix;
                });
...
function collectSuffixSets(conj_and_deriv_rules, mood_tense) {
    const suffix_sets = [];
...
function getSuffixesForLexicalExceptions(conj_and_deriv_rules, mood_tense, forms_to_stress_last_char_of_stem) {
    const exceptional_suffixes_for_mood_tense = {};
    ...
    if (forms_to_stress_last_char_of_stem?.length > 0) {
        for (const gramatical_person of forms_to_stress_last_char_of_stem) {
            ...
            if (exceptional_suffixes) {
                const changed_forms = applyToFormasConjugadas(exceptional_suffixes, (exceptional_suffix) => {
...
function getStemsForLexicalExceptions(conj_and_deriv_rules, mood_tense, stems, forms_to_stress_last_char_of_stem) {
    ...
    if (forms_to_stress_last_char_of_stem?.length > 0) {
        for (const gramatical_person of forms_to_stress_last_char_of_stem) {
            ...
            const changed_forms = applyToFormasConjugadas(stems_to_stress, (stem) => {
                const match = stem.match(last_vowel_regex);
                if (!match) {
                    throw new Error(`stem=${stem} expected to contain vowel`);
                }
...
export function getSuffixes(conj_and_deriv_rules, mood_tense, ancestor_rule_sets, rules_applied) {
...
export function getSuffixFor3p(conj_and_deriv_rules, mood_tense, ancestor_rule_sets) {
    let accumulated_suffixes = { ...ancestor_rule_sets[0].suffixes };
    const regular_suffixes = getRegularSuffixes(conj_and_deriv_rules.infinitivo_sin_prefijos, mood_tense, ancestor_rule_sets);
    const strong_pretérito_suffixes = getSuffixesForStrongPretérito(conj_and_deriv_rules, mood_tense);
...
export function maintainStressOnLastSylable(conj_and_deriv_rules, mood_tense, formas_casi_finales, rules_applied) {
