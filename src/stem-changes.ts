...
export interface StemChangeDescription {
  // If kind=="no change", the other fields are not referenced
  kind: StemChangeKind
  from: string
  to: string
...
export interface StemChangeRules extends MoodTenseMap<StemChangesForMoodTense> {
    ...
    // null is only used for mapping alternancías to verb forms.
    // A null indicates that there is no alternancía for the corresponding form.
