...
export interface VerbAspectModifications {
    ...
    // Solo para verbos de modelos de conjugacion
    stress_last_char_of_p1_stem?: boolean
    // true if the stress should be placed on the last sylable of the stem of the singular forms and the 3rd-person plural form.
    // Utilizando por ambos modelos y otros verbos, como "descafeinar"
...
export interface VerbAspectRules extends VerbAspectModifications {
    ...
    // - a MoodTense is used for a different tense in the same family
        ...
        aspects: {
            ...
            IndCond: { add_suffix_to_infinitive: true, 
                       suffixes: { s1: ["ía"],  s2: ["ías"],  s3: ["ía"],  p1: ["íamos"],  p2: ["íais"],   p3: ["ían"] } },
                                                                                                                                    // SubPres.s2            infinitivo sin *-r* + acento en la vocal final + *-s*.
            SubPres: { suffixes: { s1: ["e"],   s2: ["es"],   s3: ["e"],   p1: ["emos"],   p2: ["éis"],    p3: ["en"],     vos: [{forma:"es",uso: "Riop."}, {forma:"és",uso: "C.Am."}] } },
...
export function getInfinitiveClass(infinitivo: string) : InfinitiveClass | undefined {
    let verb_termination = infinitivo.slice(-2)
    if (verb_termination === "ír") {
        verb_termination = "ir"
    }
    if (verb_terminations_normalized.includes(verb_termination)) {
...
// This must only be called when conjugation_rules.base exists
function getParentVerb(conjugation_rules: VerbAspectRules, verb_family: InfinitiveClass, mood_tense: MoodTense) {
    ...
    mood_tense  = base_is_infinitive_class ? mood_tense : <MoodTense> base
    conjugation_rules = regular_verb_suffixes[verb_family].aspects[mood_tense]!
...
export function getRegularRules(infinitivo: string, mood_tense: MoodTense, rules_applied: VerbRulesApplied[]) : VerbAspectRules[] {
    ...
    while (conjugation_rules?.base) {
        const base_rules = getParentVerb(conjugation_rules, verb_family, mood_tense)
        ;({conjugation_rules, verb_family, mood_tense} = base_rules)
        ancestor_rule_sets.unshift(conjugation_rules)
    }
    if (mood_tense === "CmdPos") {
        const last_rule_set = ancestor_rule_sets[ancestor_rule_sets.length - 1]
...
export function getRegularSuffixes(infinitivo: string, mood_tense: MoodTense, ancestor_rule_sets: VerbAspectRules[]) : VerbConjugationSuffixes {
    ...
    ancestor_rule_sets.slice(1).forEach((ancestor_rule_sets) => {
        if (ancestor_rule_sets.suffixes) {
            ...
            conjugation_keys.forEach((key: string) => {
                const conjugation_key = <keyof VerbConjugation> key
