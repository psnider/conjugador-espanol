import type { ConjugaciónEstándarYAtípico, GrammaticalPerson, IDDeFormaConjugada } from "./index"
import type { ReglasDeConjugaciónDeVerbo } from "./verbos-con-cambios-morfológicas.js"
...
export function generaIndiceDeTemasDeFormasConjugadas() : {temas_por_deletreo: IndiceDeTemas, temas_por_fonética: IndiceDeTemas} {
 ...
 */
function findInsertionIndex(temas_del_indice: string[], consulta: string) {
    let low = 0
...
export function encuentraFormasSimilares(consulta: string) : {infinitivos: string[], formas: {[forma: string]: IDDeFormaConjugada[]} } {
    function temaCoincideConConsulta(tema: string, consulta: string) {
        if (tema) {
    ...
    const formas: {[forma: string]: IDDeFormaConjugada[]} = {}
